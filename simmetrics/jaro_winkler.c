 ...
 *
 *
 *  Based on strcmp95.c   Version 2 with modifications
...
double jaro_winkler_original_strcmp95(char *ying, char *yang, long y_length,
	...
	static char sp[39][2] = { { 'A', 'E' }, { 'A', 'I' }, { 'A', 'O' }, { 'A',
			'U' }, { 'B', 'V' }, { 'E', 'I' }, { 'E', 'O' }, { 'E', 'U' }, {
			'I', 'O' }, { 'I', 'U' }, { 'O', 'U' }, { 'I', 'Y' }, { 'E', 'Y' },
			{ 'C', 'G' }, { 'E', 'F' }, { 'W', 'U' }, { 'W', 'V' },
			{ 'X', 'K' }, { 'S', 'Z' }, { 'X', 'S' }, { 'Q', 'C' },
	...
	char ying_hold[MAX_VAR_SIZE], yang_hold[MAX_VAR_SIZE],
			ying_flag[MAX_VAR_SIZE], yang_flag[MAX_VAR_SIZE];
	...
	if (!pass) {
		...
		for (i = 0; i < 91; i++)
			for (j = 0; j < 91; j++)
				adjwt[i][j] = 0;
		for (i = 0; i < 36; i++) {
			adjwt[(int)sp[i][0]][(int)sp[i][1]] = 3;
			adjwt[(int)sp[i][1]][(int)sp[i][0]] = 3;
		}
	}

	/* If either string is blank - return - added in Version 2                    */
	if (!strncmp(ying, NULL60, y_length))
		return (0.0);
	if (!strncmp(yang, NULL60, y_length))
		return (0.0);
	...
	if (!ind_c[1]) {
		for (i = 0; i < ying_length; i++)
			if (islower(ying_hold[i]))
				ying_hold[i] -= 32;
		for (j = 0; j < yang_length; j++)
			if (islower(yang_hold[j]))
				yang_hold[j] -= 32;
	}
			...
			if (ying_flag[i] == ' ' && INRANGE(ying_hold[i])) {
				for (j = 0; j < yang_length; j++) {
					if (yang_flag[j] == ' ' && INRANGE(yang_hold[j])) {
						if (adjwt[(int)ying_hold[i]][(int)yang_hold[j]] > 0) {
							N_simi += adjwt[(int)ying_hold[i]][(int)yang_hold[j]];
							yang_flag[j] = '2';
							break;
						}
					}
