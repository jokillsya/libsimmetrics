...
float custom_smith_waterman_gotoh(const char *str1, const char *str2, sw_gotoh_t conf) {
	...
	for (i = 0; i < n; i++) {
		...
		cost = conf.sub_cost.cost_func(str1, i, str2, 0);
		...
		if (i == 0)
			...
			for (k = w_st; k < i; k++)
				max_gap_cost = MAX(max_gap_cost, d[i - k][0] - conf.gap_cost.cost_func(i - k, i));
	...
	for (j = 0; j < m; j++) {
		...
		cost = conf.sub_cost.cost_func(str1, 0, str2, j);
		...
		if (j == 0)
			...
			for (k = w_st; k < j; k++)
				max_gap_cost = MAX(max_gap_cost, d[0][j - k] - conf.gap_cost.cost_func(j - k, j));
	...
	for (i = 1; i < n; i++) {
		...
		for (j = 1; j < m; j++) {
			...
			cost = conf.sub_cost.cost_func(str1, i, str2, j);
			w_st = i - conf.win_size;
			...
			for (k = w_st; k < i; k++)
				max_gap_cost1 = MAX(max_gap_cost1, d[i - k][j] - conf.gap_cost.cost_func(i - k, i));
			...
			for (k = w_st; k < j; k++)
				max_gap_cost2 = MAX(max_gap_cost2, d[i][j-k] - conf.gap_cost.cost_func(j - k, j));
...
float custom_smith_waterman_gotoh_similarity(const char *str1, const char *str2, sw_gotoh_t conf) {
	...
	float max_value = MIN(strlen(str1), strlen(str2));
	float ret;

	if(conf.sub_cost.cost.max_cost > -conf.gap_cost.cost.max_cost)
		max_value *= conf.sub_cost.cost.max_cost;
	else
		max_value *= -conf.gap_cost.cost.max_cost;
